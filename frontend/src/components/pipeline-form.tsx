...
export function PipelineForm({ topic, onTopicChange, onStart, loading }: Props) {
  return (
    <section className="bg-panel border border-border rounded-xl p-6 shadow-card">
      ...
      <textarea
        className="w-full bg-surface border border-border rounded-lg p-3 text-sm min-h-[100px] focus:outline-none focus:border-accent"
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {["linkedin", "substack", "medium", "x", "instagram"].map((p) => (
          <span key={p} className="text-xs px-2 py-1 rounded bg-surface border border-border text-muted">
            {p}
          </span>
        ))}
      </div>
